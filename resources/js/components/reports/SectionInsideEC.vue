...
import { emptyInsideEC } from '@/composables/useReportCalculations';
import type { Barangay, InsideEvacuationCenter } from '@/types';
...
const rows = defineModel<InsideEvacuationCenter[]>('rows', { required: true });
...
function clampCum(event: Event, row: InsideEvacuationCenter, field: 'families' | 'persons') {
...
function clampNow(event: Event, row: InsideEvacuationCenter, field: 'families' | 'persons') {
    const input = event.target as HTMLInputElement;
    const cumKey = field === 'families' ? 'families_cum' : 'persons_cum';
    const nowKey = field === 'families' ? 'families_now' : 'persons_now';
    if (row[nowKey] > row[cumKey]) {
...
<template>
    <div class="space-y-4">
        <div class="flex items-center justify-between">
            <h3 class="text-lg font-semibold text-slate-900">III-A. Inside Evacuation Centers</h3>
            ...
            <table class="min-w-full divide-y divide-slate-200 border">
                <thead class="bg-indigo-50">
                    <tr>
                        ...
                        <th class="px-3 py-2 text-left text-xs font-medium tracking-wider text-slate-500">Brgy of EC</th>
                        <th class="px-3 py-2 text-left text-xs font-medium tracking-wider text-slate-500">Evacuation Center</th>
                        ...
                        <th class="px-3 py-2 text-left text-xs font-medium tracking-wider text-slate-500">Remarks</th>
                ...
                <tbody class="divide-y divide-slate-200 bg-white">
                    ...
                    <tr v-if="rows.length === 0">
                        <td colspan="10" class="px-4 py-4 text-center text-sm text-slate-500">
                            No evacuation center entries. Click "+ Add Row" to add one.
                        </td>
                    </tr>
                </tbody>
