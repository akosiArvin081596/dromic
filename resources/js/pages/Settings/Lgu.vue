...
const ldrrmcLogoFile = ref<File | null>(null);
const ldrrmcLogoPreview = ref<string | null>(props.settings?.ldrrmc_logo_url ?? null);
const removeLdrrmcLogo = ref(false);
...
function onLdrrmcLogoChange(event: Event) {
    const file = (event.target as HTMLInputElement).files?.[0];
    if (file) {
        ldrrmcLogoFile.value = file;
        removeLdrrmcLogo.value = false;
        ldrrmcLogoPreview.value = URL.createObjectURL(file);
    }
}

function clearLdrrmcLogo() {
    ldrrmcLogoFile.value = null;
    ldrrmcLogoPreview.value = null;
    removeLdrrmcLogo.value = true;
}
...
function submit() {
    ...
    if (ldrrmcLogoFile.value) {
        formData.append('ldrrmc_logo', ldrrmcLogoFile.value);
    }
    if (removeLdrrmcLogo.value) {
        formData.append('remove_ldrrmc_logo', '1');
    }
        ...
        <div class="mx-auto max-w-5xl px-4 py-6 sm:px-6 lg:px-8">
            ...
            <form class="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-2" @submit.prevent="submit">
                ...
                <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-800">
                    ...
                    <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Left side of report header. Max 2MB</p>
                ...
                <!-- LDRRMC Logo -->
                <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-800">
                    <h2 class="text-lg font-semibold text-slate-900 dark:text-slate-100">LDRRMC Logo</h2>
                    <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Right side of report header. Max 2MB</p>
                    <div class="mt-4 flex items-center gap-6">
                        <div
                            class="flex h-24 w-44 shrink-0 items-center justify-center overflow-hidden rounded-lg border border-slate-200 bg-slate-50 dark:border-slate-600 dark:bg-slate-700"
                        >
                            <img v-if="ldrrmcLogoPreview" :src="ldrrmcLogoPreview" alt="LDRRMC Logo" class="h-full w-full object-contain" />
                            <span v-else class="text-xs text-slate-400">No logo uploaded</span>
                        </div>
                        <div class="flex flex-col gap-2">
                            <label
                            ...
                            >
                                Choose File
                                <input type="file" accept="image/png,image/jpeg,image/webp" class="hidden" @change="onLdrrmcLogoChange" />
                            </label>
                            <button
                                v-if="ldrrmcLogoPreview"
                                type="button"
                                class="text-left text-xs text-red-600 hover:text-red-800"
                                @click="clearLdrrmcLogo"
                            >
                                Remove logo
                            </button>
                        </div>
                    </div>
                    <p v-if="errors.ldrrmc_logo" class="mt-2 text-xs text-red-600">{{ errors.ldrrmc_logo }}</p>
                </div>
